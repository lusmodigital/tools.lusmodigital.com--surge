(function () {
	...
	var cck = function(n,v,d) {
		...
		if (d) {
			...
			e = "; expires=" + _d.toUTCString() + "; SameSite=None; Secure";
		}
	...
	var rq = function( t , p , cb ){ // request ajax
		...
		var rq = tr.getDomainUrl() + "/stvid/track.php";
	...
	var tr = {
		domain_scripts : 'marketbold.com',
		force_https:false,
		page_is : '',
		isOptin: function( v ){
			v = v || false;
			if( !v ){
				return tr.page_is == 'optin';
			}
			
			var ct = document.referrer;			
			if( tr.getParameterByName('utm_source') ){
				ct = tr.getParameterByName('utm_source');
			}
			
			cck( "_OAttr" , ct , 360 );
			tr.page_is = 'optin';
		},
		isOptinThankYou: function( v ){
			v = v || false;
			if( !v ){
				return tr.page_is == 'optinty';
			}
			tr.page_is = 'optinty';
		},
		...
		getDomainUrl:function(){
			if ( tr.force_https ){
				return "https://" + tr.domain_scripts;
			}else{
				return window.location.protocol + "//" + tr.domain_scripts;
			}
		},
		
		createWorker: function(workerUrl){
			var worker = null;
			try {
				...
				try {
					...
					try {
						blob = new Blob([
							...
							//'worker={},onmessage=function(t){var a=t.data;if(void 0!==a.action)switch(a.action){case"timeout":var e=a.delay;setTimeout(function(){postMessage({status:1})},e)}};'
		...
		init: function(){
			...
			tr.worker = tr.createWorker(tr.getDomainUrl() +"/stvid/js/worker.js?n="+(new Date().getTime()));
			...
			//ct();
			tr.worker.onmessage = function(e) {		
				if( e.data.status == 1 ){
					if( TimeMe.active ){
						rq(
							tr.rq_t,
		...
		rq: function( succ ){
			...
			var c = cmpgn();
			var attr = "";
			var hasAttr = tr.getParameterByName('attr');
			if( hasAttr ){
				cck( "_hasAttr" , hasAttr , 360 );
			}
			if( rck("_hasAttr") ){
				attr = rck("_hasAttr");
			}
			var oattr = "";
			if( tr.isOptinThankYou() && rck('_OAttr') ){
				oattr = rck('_OAttr');
			}
			rq(
				tr.rq_t,
	...
	var _tck = function( p ){
		...
		if( nt != 'nt' ){
			...
			if( typeof(p.force_https) == 'boolean' ){
				tr.force_https = p.force_https;
			}
